...
interface NavbarProps {
  ...
  onSignOut?: () => void;
  onSwitchAccount?: (type: 'awa' | 'admin' | 'user_b') => void;
}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2.5">
            ...
            >
              ...
              <span>
                {isAdminMode ? 'Admin Command' : `Participant (${displayName})`}
              </span>
            </button>
            ...
            <div className="relative">
              ...
              >
                <div className="w-7 h-7 rounded-lg bg-blue-900 text-white text-xs font-bold flex items-center justify-center">
                  {initials}
                </div>
              ...
              {/* User Dropdown */}
              {showUserMenu && (
                <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-200 rounded-2xl shadow-xl z-50 p-2 space-y-2 animate-in fade-in zoom-in-95 duration-150 text-xs">
                  <div className="p-3 bg-slate-50 rounded-xl">
                    <p className="font-bold text-slate-900">{displayName}</p>
                    <p className="text-[11px] text-slate-700 truncate">{userProfile?.email || 'Authenticated User'}</p>
                    <span className="inline-block mt-1 text-[10px] font-semibold text-emerald-800 bg-emerald-100 px-1.5 py-0.5 rounded">
                      {isAdminMode ? 'Admin Role' : 'Participant Role'}
                    </span>
                  </div>

                  <div className="space-y-1">
                    <span className="text-[10px] font-bold uppercase text-slate-700 px-2">Quick Switch Demo</span>
                    <button
                      onClick={() => {
                    ...
                    >
                      <span>Awa Diop (AI Track)</span>
                      {userProfile?.name?.includes('Awa') && <CheckCircle2 className="w-3.5 h-3.5 text-blue-600" />}
                    </button>
                    <button
                      onClick={() => {
                        setShowUserMenu(false);
                        onSwitchAccount?.('admin');
                      }}
                      className="w-full text-left px-2.5 py-1.5 rounded-lg hover:bg-blue-50 text-slate-700 flex items-center justify-between"
                    >
                      <span>Dr. Aminata Touré (Admin)</span>
                      {userProfile?.name?.includes('Aminata') && <CheckCircle2 className="w-3.5 h-3.5 text-blue-600" />}
                    </button>
                    <button
                      onClick={() => {
                        setShowUserMenu(false);
                        onSwitchAccount?.('user_b');
                      }}
                      className="w-full text-left px-2.5 py-1.5 rounded-lg hover:bg-blue-50 text-slate-700 flex items-center justify-between"
                    >
                      <span>Kofi Annan (Embedded Track)</span>
                      {userProfile?.name?.includes('Kofi') && <CheckCircle2 className="w-3.5 h-3.5 text-blue-600" />}
                    </button>
                  </div>
